...
| UART | Boot and protocol traffic traverse the linked UART path; fixed raw UART PUTCH bytes remain identical to official MOS | `emos-vdu-check`, `test_emos_vdu` | Fab models a 16-byte TX FIFO and an explicitly arbitrary 4x RX slowdown. Its VDP link exchanges complete bytes without an independent endpoint baud clock, so a wrong BRG divisor changes emulator delay but cannot create the baud mismatch or physical line errors seen by hardware; divisor, error flags, and timing remain linked-image or hardware cells |
