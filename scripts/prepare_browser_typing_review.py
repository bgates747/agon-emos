#!/usr/bin/env python3
"""Build the browser typing SD fixture and check its shared EMOS UART round trip."""
...
def main():
    ...
    parser.add_argument('--fixture', choices=('typing',), default='typing')
    parser.add_argument('--packets', type=Path, help='Retained P4 sender host-test output for wire fixture')
    args = parser.parse_args()
    if args.fixture == 'typing' and not args.packets:
        parser.error('--fixture typing requires --packets')
    ...
    project = ROOT/'projects/browser-typing'
    name = 'BTYPE'
    peer_source = ROOT/'scripts/browser_typing_peer.py'
    identity = (project/'identity.txt').read_text().strip()
    status_path = project/'status.txt'
    status = status_path.read_text().strip() if args.fixture == 'typing' else 'draft'
    ...
    if args.fixture == 'typing':
        ...
        shutil.copy2(args.packets, media/'typing-packets.bin')
        shutil.copy2(args.packets.with_suffix('.json'), media/'typing-packets.json')
    ...
    source_files = [project/'Makefile', project/'identity.txt', *sorted((project/'src').iterdir()),
                    Path(__file__).resolve(), peer_source, ROOT/'scripts/review_boot.py']
    if args.fixture == 'typing': source_files.append(status_path)
    ...
    (output/'fixture-inputs.json').write_text(json.dumps({
        ...
        'sender_packets_sha256': digest(media/'typing-packets.bin') if args.fixture == 'typing' else None,
        'sender_provenance_sha256': digest(media/'typing-packets.json') if args.fixture == 'typing' else None,
    ...
    make_profile(output/'profile', outputs['firmware'], outputs['firmware_map'], media, fab,
                 runtime=runtime, peer=peer, mutable_names=("typing-state.bin",))
