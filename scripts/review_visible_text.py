#!/usr/bin/env python3
"""Check target no-peer visible text failure, then prepare an isolated graphical review.
...
def main():
    ...
    data = bytearray()
    expected = b'VDP TEXT FAIL:'
    expected_reasons = (b'VDP TEXT FAIL: transmit timeout', b'VDP TEXT FAIL: no reply from EDP')
    ...
    if data.count(expected) != 1 or not data.rstrip().endswith(b'/ *'):
        raise SystemExit('FAIL: expected one no-peer visible text failure and prompt return; see no-peer.log')
    if b'VDP TEXT PASS' in data or sum(reason in data for reason in expected_reasons) != 1:
    ...
    print('PASS: emulated eZ80 no-peer visible text failure and prompt return')
