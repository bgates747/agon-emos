#!/usr/bin/env python3
"""Check target no-peer flow failure, then prepare an isolated graphical review.
...
def main():
    ...
    data = bytearray()
    expected = b'UART FLOW FAIL:'
    expected_reasons = (b'UART FLOW FAIL: CTS did not release', b'UART FLOW FAIL: peer did not hold CTS')
    ...
    if data.count(expected) != 1 or not data.rstrip().endswith(b'/ *'):
        raise SystemExit('FAIL: expected one no-peer flow failure and prompt return; see no-peer.log')
    if b'UART FLOW PASS' in data or sum(reason in data for reason in expected_reasons) != 1:
    ...
    print('PASS: emulated eZ80 no-peer flow failure and prompt return')
