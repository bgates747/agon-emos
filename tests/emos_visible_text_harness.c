...
BYTE uart1_try_put(BYTE b){
 const BYTE req[]={12,31,2,2,69,77,79,83,32,84,79,32,69,68,80,58,32,85,65,82,84,32,84,69,88,84,13,10,23,0,202,23,0,128,166};
 if(mode==6 || (mode==13 && ++attempts<6))return UART_POLL_BLOCKED;
 if(mode==7)return UART_POLL_ERROR;
 assert(sent<35 && b==req[sent++]);return UART_POLL_READY;
}
BYTE uart1_try_get(BYTE *b){
 const BYTE ack[]={0x80,1,0xA6};assert(sent==35);
 ...
 if(mode==4){*b=0xA6;return UART_POLL_READY;}
...
int main(void){
 for(mode=0;mode<14;++mode){
  ...
  assert(emos_visible_text()==(mode==0 || mode==13));assert(closed==(mode==9||mode==12?0:1));
  assert(!claimed);assert(serialFlags==(mode==12?0x10:0));
 }
 puts("14 visible text command scenarios passed");
}
